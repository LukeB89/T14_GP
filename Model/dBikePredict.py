...
def bikesPredict(station,day,hour):
    ...
    with engine.connect() as conn:
        ...
        weather_data = """
            SELECT wf.id, wf.main_temp, wf.main_humidity, wf.wind_speed 
            FROM dublinbikes.weather_forecast as wf, dublinbikes.bike_weather_assoc as bw 
            WHERE bw.weather_station = wf.name AND bw.bike_station_id = {} AND wf.day = {} 
            AND wf.hour = {};""".format(station, day, forecast_hour)
        ...
        noCov_insert = """
            INSERT INTO dublinbikes.station_day_prediction (number,day,covid,pred_h_{}) 
            VALUES ({}, {}, {}, {});""".format(hour, station, day, 0, predicted_bikes_noCov[0])

        noCov_update = """
            UPDATE dublinbikes.station_day_prediction SET pred_h_{} = {} 
            WHERE (number = {}) AND (day = {}) 
                AND (covid = {});""".format(hour, predicted_bikes_noCov[0], station, day, 0)

        cov_insert = """
            INSERT INTO dublinbikes.station_day_prediction (number,day,covid,pred_h_{}) 
            VALUES ({}, {}, {}, {});""".format(hour, station, day, 1, predicted_bikes_cov[0])

        cov_update = """
            UPDATE dublinbikes.station_day_prediction SET pred_h_{} = {} 
            WHERE (number = {}) AND (day = {}) 
                AND (covid = {});""".format(hour, predicted_bikes_cov[0], station, day, 1)
